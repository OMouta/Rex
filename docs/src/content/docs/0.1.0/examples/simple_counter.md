...
This example demonstrates the core concepts of Rex through a simple counter application. You'll learn about reactive state, computed values, event handling, and component lifecycle.

## Basic Counter

Let's start with a minimal counter implementation:
...
local function CounterWithEffects()
    ...
    Rex.useEffect(function()
        ...
        -- Update total clicks when count increases
        if currentCount > 0 then
            totalClicks:update(function(current) return current + 1 end)
        end
    end, {count})
